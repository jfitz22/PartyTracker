...
export function AddItemDialog({ characterId, open, onOpenChange, editingItem }: AddItemDialogProps) {
  ...
  const onSubmit = (data: FormValues) => {
    // Clean up empty strings to nulls for the API
    const payload = {
      ...data,
      imageUrl: data.imageUrl || null,
      // If maxCharges is provided, ensure currentCharges matches on creation.
      // The backend schema allows currentCharges, we'll set it same as max.
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border border-border p-3 shadow-sm bg-background">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>Is this currently worn/wielded?</FormDescription>
                      </div>
                      <FormControl>
